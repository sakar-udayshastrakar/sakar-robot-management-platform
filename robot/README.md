...
**Status note (documentation sync, 2026-09-22): the line below describes this directory's state at the time of the original workspace-organization copy and is now stale — substantial Android agent development has happened here since (MQTT command dispatch/idempotency, `GO_TO_POINT`/`RETURN_TO_DOCK` real Peanut SDK executors gated behind `OperatingMode.HARDWARE_TEST`, a `START_TASK` software simulation, a `:virtual-agent` simulator module, and more). See the root `README.md`'s "Current Project Status" (Android Agent row) and `SakarC40Agent/README.md` for the current, accurate account. No connection to a physical C40 has occurred at any point.**

*Original note, as of the workspace-organization task:* "Current phase: DOCUMENTATION / ARCHITECTURE PREPARATION. No new code has been added to this copy; no feature implementation, no Peanut SDK modification, and no connection to a physical C40 has occurred as part of this workspace-organization task."
