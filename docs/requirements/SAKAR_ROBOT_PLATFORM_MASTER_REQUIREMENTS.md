...
**Current Implementation Status (added by a documentation-synchronization pass, 2026-09-22 — this note only, nothing else on this page was changed by that pass):** this remains a requirements/design document — the requirements, architecture decisions, and gradings above and throughout are preserved as originally written and are **not** updated here to reflect what has since been built. A large amount of implementation has happened since this document's v2.0 date (2026-08-26): authentication/RBAC, robot registry, Keenon Cloud integration, task/cleaning orchestration (including a Task-panel `START`/`STOP` bridge to real vendor dispatch), MQTT telemetry and command dispatch, the Android agent's command-consumption loop, and most of the web frontend are now real, tested code — none of it existed when this document was written, and the `IMPLEMENTED` status this document deliberately avoids using is now genuinely applicable to those areas in the actual codebase. Remote lock/unlock (Part 11) remains exactly as graded here: SDK-level APIs `CONFIRMED`, physical effect `REQUIRES PHYSICAL C40 TEST`, not yet performed. For the current, evidence-based status of what is actually implemented, test-verified, or still missing, see the root `README.md`'s "Current Project Status" section, which is maintained as the living source of truth and is updated far more frequently than this requirements document.
