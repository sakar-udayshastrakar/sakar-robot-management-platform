...
export function ErrorsPage() {
  ...
  const filtered = errors.filter(
    (e) => (statusFilter === 'ALL' || e.status === statusFilter) &&
      (!search.trim() || e.message.toLowerCase().includes(search.trim().toLowerCase()) || e.errorCode.toLowerCase().includes(search.trim().toLowerCase())),
  );

  return (
    <div>
      <PageHeader title="Errors" subtitle="Robot error management." />
