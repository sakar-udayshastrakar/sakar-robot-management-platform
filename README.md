...
| Field | Value |
|---|---|
| Current phase | Phase 7 — Robot Agent Command Loop (`SakarC40Agent` command consumption + result reporting) implemented on top of Phase 6, uncommitted |
| Overall status | Backend + Agent MQTT pipeline implemented, live-broker-validated, test-verified; Phase 6 approved-scope backend work implemented and test-verified; Phase 7 closes the agent-side half of the command loop (software simulation only, no Peanut SDK cleaning API exists — see above); Web Application now consumes every Phase 6 endpoint for real (Users/Roles/Tasks/Alerts/Cleaning/Commands), plus the pre-existing real auth/org/site/robot/audit wiring; three product-expansion modules (Open Platform/OTA/IoT-Elevator) remain requirements-only, not implemented; not yet committed/merged; mobile, physical robot validation, and live-browser verification of the frontend pass not performed (no local Postgres/Redis/Docker available) |
| Last updated | 2026-08-30 |
| Latest branch | `feature/robot-agent-command-loop` (created off `feature/phase6-fleet-operations`; working tree modified, not committed — see `git status`/`git diff` before trusting "current" claims) |
| Latest commit | `2650ab5` — feat: implement phase 6 fleet operations (Phase 7 "Robot Agent Command Loop" work sits on top, uncommitted) |
| Test status | **102 / 102 automated backend tests passing** (`cd backend && ./mvnw clean test`) — 93 pre-existing (Phase 1/3/6) + 8 Phase 7 `START_TASK` tests + 1 Phase 7 `RETURN_TO_DOCK` test (`CommandResultIngestionServiceTest`, `CommandExpiryServiceTest`). **28 / 28 automated frontend tests passing** (`cd web && npm run test -- --run`) — unchanged this pass. **37 / 37 automated Robot Agent (`:api` module) tests passing** (`cd robot/SakarC40Agent && ./gradlew :api:test`) — 18 pre-existing + 10 Phase 7 `START_TASK` tests + 9 Phase 7 `RETURN_TO_DOCK` tests (`PeanutSdkReturnToDockExecutorTest`, `CompositeRobotCommandExecutorTest`, two new `CommandDispatcherTest` cases) |
| Build status | Backend: **`./mvnw -o test` and `compile`/`test-compile` successful**. Frontend: unchanged this pass. Robot Agent: **`:api:test` and `:app:compileDebugJavaWithJavac` both BUILD SUCCESSFUL** (the `:app` module — where `SimulatedRobotCommandExecutor` AND the real `RealReturnToDockGateway` live — was compiled against the real local Android SDK, not just the plain-Java `:api` module) |
...
| SakarC40Agent integration | 🔄 Partially Implemented (Phase 3 + 7) | MQTT telemetry/heartbeat/events/errors link exists (`:api` module) and is wired into `SakarC40Application`; Phase 7 adds inbound command consumption (`CommandDispatcher`, routed by `CompositeRobotCommandExecutor`) and lifecycle result reporting for two command types: `START_TASK` (`SimulatedRobotCommandExecutor`, a software placeholder, not a Peanut SDK call) and `RETURN_TO_DOCK` (`PeanutSdkReturnToDockExecutor`, a REAL `BatteryComponent.autoCharge()` call, gated by `OperatingMode.HARDWARE_TEST`); `STOP_TASK`/`PAUSE_TASK`/`RESUME_TASK` have no executor at all yet; not yet run against a real broker or physical robot — see Robot Integration Status |
...
**Current authorized scope:** backend Phase 1 is complete; Story 3 (`SakarC40Agent` MQTT communication layer), Phase 6 (Users/Roles, Tasks, Alerts, Cleaning history, non-lock Remote Commands), and Phase 7 (Robot Agent Command Loop — agent-side command consumption and result reporting for `START_TASK` (software simulation) and `RETURN_TO_DOCK` (real Peanut SDK call, `OperatingMode.HARDWARE_TEST`-gated)) are implemented in software and test-verified, per the roadmap above, but **uncommitted** on `feature/robot-agent-command-loop` (branched off the already-committed `feature/phase6-fleet-operations`) — pending review/approval before any commit, merge, or push. Open Platform / OTA / IoT-Elevator Integration are requirements-only (see Proposed Product Expansion above) and remain explicitly not implemented pending separate review. Physical C40 connection, physical robot control (including `RETURN_TO_DOCK`'s now-real SDK call), and remote lock/unlock certification remain out of scope until the separate physical-validation go-ahead described in Master Requirements Part 33/38 is given, and until a business decision is made on how `SimulatedRobotCommandExecutor`'s remaining command types (`START_TASK`, and any future cleaning-control commands) should eventually be replaced (see Roadmap above).
