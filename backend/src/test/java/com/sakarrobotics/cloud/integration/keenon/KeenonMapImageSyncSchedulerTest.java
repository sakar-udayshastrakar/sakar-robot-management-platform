...
/**
 * Verifies {@link KeenonMapImageSyncScheduler} is gated on {@code
...
class KeenonMapImageSyncSchedulerTest {
    ...
    @Mock
    private KeenonMapImageSyncService keenonMapImageSyncService;

    private KeenonMapImageSyncScheduler scheduler() {
        return new KeenonMapImageSyncScheduler(robotRepository, robotModelRepository, robotCapabilityService,
                keenonMapImageSyncService);
    }
    ...
    void syncAll_syncsEveryEligibleKeenonCloudRobot_gatedOnGetStatus_notGetMap() {
        ...
        verify(keenonMapImageSyncService).sync(robot);
    ...
    void syncAll_skipsDeactivatedRobots() {
        ...
        verifyNoInteractions(keenonMapImageSyncService);
    }
    ...
    void syncAll_oneRobotsSyncFailure_doesNotAbortTheOthers() {
        ...
        when(robotModelRepository.findById(model.getId())).thenReturn(Optional.of(model));
        when(robotCapabilityService.isSupported(model.getId(), RobotCapabilityType.GET_STATUS)).thenReturn(true);
        when(keenonMapImageSyncService.sync(failingRobot))
                .thenThrow(new ApiException(SakarErrorCode.VENDOR_API_ERROR, "Keenon Open Platform request failed"));

        scheduler().syncAll();

        verify(keenonMapImageSyncService).sync(failingRobot);
        verify(keenonMapImageSyncService).sync(healthyRobot);
    }
    ...
    void syncAll_oneRobotsUnexpectedFailure_doesNotAbortTheOthers() {
        ...
        when(robotModelRepository.findById(model.getId())).thenReturn(Optional.of(model));
        when(robotCapabilityService.isSupported(model.getId(), RobotCapabilityType.GET_STATUS)).thenReturn(true);
        when(keenonMapImageSyncService.sync(failingRobot)).thenThrow(new RuntimeException("unexpected"));
    ...
    void scheduledSync_whenDisabled_neverRunsASync() throws ReflectiveOperationException {
        KeenonMapImageSyncScheduler scheduler = scheduler();
        setEnabled(scheduler, false);

        scheduler.scheduledSync();

        verifyNoInteractions(robotRepository, keenonMapImageSyncService);
    }
    ...
    private static void setEnabled(KeenonMapImageSyncScheduler scheduler, boolean value) throws ReflectiveOperationException {
        var field = KeenonMapImageSyncScheduler.class.getDeclaredField("enabled");
