...
/**
 * Verifies {@link KeenonMapMetadataSyncScheduler} only syncs KEENON_CLOUD
 * robots with the GET_STATUS capability, skips deactivated robots, and
...
class KeenonMapMetadataSyncSchedulerTest {
    ...
    private KeenonMapMetadataSyncScheduler scheduler() {
        return new KeenonMapMetadataSyncScheduler(robotRepository, robotModelRepository, robotCapabilityService,
                keenonMapMetadataSyncService);
    }
    ...
    void syncAll_syncsEveryEligibleKeenonCloudRobot() {
        ...
        verify(keenonMapMetadataSyncService).sync(robot);
    }
    ...
    void syncAll_skipsDeactivatedRobots() {
        ...
        verifyNoInteractions(keenonMapMetadataSyncService);
    }
    ...
    void syncAll_oneRobotsSyncFailure_doesNotAbortTheOthers() {
        ...
        when(robotModelRepository.findById(model.getId())).thenReturn(Optional.of(model));
        when(robotCapabilityService.isSupported(model.getId(), RobotCapabilityType.GET_STATUS)).thenReturn(true);
        when(keenonMapMetadataSyncService.sync(failingRobot))
                .thenThrow(new ApiException(SakarErrorCode.VENDOR_API_ERROR, "Keenon Open Platform request failed"));

        scheduler().syncAll();

        verify(keenonMapMetadataSyncService).sync(failingRobot);
        verify(keenonMapMetadataSyncService).sync(healthyRobot);
    }
    ...
    void syncAll_oneRobotsUnexpectedFailure_doesNotAbortTheOthers() {
        ...
        when(robotModelRepository.findById(model.getId())).thenReturn(Optional.of(model));
        when(robotCapabilityService.isSupported(model.getId(), RobotCapabilityType.GET_STATUS)).thenReturn(true);
        when(keenonMapMetadataSyncService.sync(failingRobot)).thenThrow(new RuntimeException("unexpected"));
    ...
    void scheduledSync_whenDisabled_neverRunsASync() throws ReflectiveOperationException {
        KeenonMapMetadataSyncScheduler scheduler = scheduler();
        setEnabled(scheduler, false);

        scheduler.scheduledSync();

        verifyNoInteractions(robotRepository, keenonMapMetadataSyncService);
    }
    ...
    private static void setEnabled(KeenonMapMetadataSyncScheduler scheduler, boolean value) throws ReflectiveOperationException {
        var field = KeenonMapMetadataSyncScheduler.class.getDeclaredField("enabled");
